...
class Tco(nn.Module):
    def __init__(self,net,validate_loader,drop_p = 0.8,s_l = 0.7,gamma = 0.1,steps = 22,lr = 1e-4,weight_decay = 1e-4,momentum=0.9,solver_name="doubleSGD"):
    ...
    def forward(self,net):
        ...
        for imgs, _none in val_bar:
           self.solver_other.zero_grad()
           imgs = imgs.to('mps')
           imgs,er = self.select(imgs,net)
           if not er:
               continue
           net.train(True)
           output = net(imgs)
