...
parser.add_argument("--threshold_acc",type=int,default=-1)
parser.add_argument("--margin",type=float)
parser.add_argument("--tag",type=str,choices=['max','margin'],help='choose one of the followed options: max, margin',default='max')
...
class PseudoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #     if image.ndim == 3 and image.shape[0] in [1, 3]:  # 如
...
def main():
    ...
    if dataset == 'cifar10':
        data_root = os.path.abspath(os.path.join(os.getcwd(), '../cifar10'))  # get data root path
    ...
    for run in range(5):
        ...
        if tco_switch:
            ...
            Tco_mode = Tco(net = net,validate_loader = validate_loader,max_confidence = max_confidence,min_confidence = min_confidence,margin = margin,lr = base_lr,tag=args.tag)
        ...
        for epoch in range(epochs):
           ...
           print('epoch: {}\ttrain_loss: {:.4f}\ttrain_acc: {:.2f}%\tval_acc: {:.2f}%\ttest_acc: {:.2f}%'.format(epoch + 1,
                                                                                                                 avg_train_loss_epoch,
                                                                                                                 train_acc_epoch,
                                                                                                                 val_acc_epoch,
                                                                                                                 test_accuracy))
           # if  tco_switch and best_acc_val > threshold_acc:
           if tco_switch and ((best_acc_val > threshold_acc) or (epoch > epochs/2 and avg_train_loss_epoch > last_loss) or tag):  # 判断模型是否收敛？
               ...
               print('using TCO!!!' + '***' * 10)
               imgs, labels_pseudo = Tco_mode(net, current_lr, alpha, allocated, batch_size=batch_size, topN=topN)
               pseudo_dataset = PseudoDataset(imgs, labels_pseudo, transform = data_transform['train'])  # 构建伪标签数据集
           ...
           # print('epoch: {}\ttrain_loss: {:.4f}\ttrain_acc: {:.2f}%\tval_acc: {:.2f}%\ttest_acc: {:.2f}%'.format(epoch + 1, avg_train_loss_epoch, train_acc_epoch, val_acc_epoch, test_accuracy))
           ...
           if val_acc_epoch < last_acc - 10 and count_backtrack < 3:
                net.load_state_dict(model_checkpoint)
                optimizer.load_state_dict(optimizer_checkpoint)
                schedule.load_state_dict(scheduler_checkpoint)
                count_backtrack += 1
                print("--------------------------------------The model weights are backtracked-------------------------------------")
           else:
                count_backtrack = 0
                model_checkpoint = net.state_dict()
                optimizer_checkpoint = optimizer.state_dict()
                scheduler_checkpoint = schedule.state_dict()
                last_acc = val_acc_epoch
           last_loss = avg_train_loss_epoch
        print('Run {}  ----- best_acc_train:{:.4f}\tbest_acc_val:{:.4f}%\tbest_acc_test:{:.4f}%'.format(run+1, best_acc_train, best_acc_val, best_acc_test))
    print("Finish!!!")
