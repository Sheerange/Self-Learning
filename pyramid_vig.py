...
class DeepGCN(torch.nn.Module):
    ...
    def forward(self, inputs, pred=None):
        ...
        if self.aug and pred is not None:
            # print(x.shape)
            x = self.prediction[:4](x).squeeze(-1).squeeze(-1)   # x: [64, 1024]
            x = forward_aug(self, x, proxies=self.prediction[4].weight.reshape(-1,x.shape[-1]).data, pred=pred)
            x = self.prediction[4](x.unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1)
            # x = self.prediction(x).squeeze(-1).squeeze(-1)
