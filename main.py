...
parser.add_argument("--max_confidence",type=float)
parser.add_argument("--min_confidence",type=float)
parser.add_argument("--tco",type=int)
parser.add_argument("--threshold_acc",type=int)
parser.add_argument("--margin",type=float)
parser.add_argument("--tag",type=str,choices=['max','margin'],help='choose one of the followed options: max, margin')
...
def main():
    ...
    for epoch in range(epochs):
       ...
       if tco_switch and ((best_acc_val > threshold_acc) or (epoch > epochs/2 and avg_train_loss_epoch > last_loss) or tag):  # 判断模型是否收敛？
           ...
           print('using TCO!!!' + '***' * 10)
           net = Tco_mode(net, current_lr, alpha, allocated)
       ...
       print(
           'epoch: {}\ttrain_loss: {:.4f}\ttrain_acc: {:.2f}%\tval_acc: {:.2f}%\ttest_acc: {:.2f}'.format(epoch + 1, avg_train_loss_epoch,
                                                                                        train_acc_epoch, val_acc_epoch, test_accuracy))
    ...
    print('best_acc_train:{:.4f}\tbest_acc_val:{:.4f}%\tbest_acc_test:{:.4f}%'.format(best_acc_train, best_acc_val, best_acc_test))
    print("Finish!!!")
