...
def get_data(Path, img_size, batch_size, dataset_name):
  ...
  if dataset_name == "CUB":      
      ...
      Path = Path + dataset_name
      cifar100_training = torchvision.datasets.CIFAR100(root=Path, train=True, download=True, transform=transforms.Compose(train_transform_list))
      cifar100_testing = torchvision.datasets.CIFAR100(root=Path, train=False, download=True, transform=transforms.Compose(test_transforms_list))
